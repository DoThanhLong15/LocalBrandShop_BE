...
public class ApiCartController {
    ...
    public ResponseEntity<Object> addToCart(@Valid @RequestBody Cart cart,
        ...
        if (!this.cartService.checkQuantityValid(cart.getProductId(), cart)) {
            return this.errorResponseUtil.buildErrorResponse("cart.quantity.invalid.errMsg", locale, HttpStatus.BAD_REQUEST);
        }

        if (!this.cartService.checkPriceValid(cart.getProductId(), cart)) {
    ...
    public ResponseEntity<Object> updateCart(@RequestBody CartDTO cartDTO,
            @PathVariable(value = "cartId") int cartId, Principal principal, Locale locale) {
